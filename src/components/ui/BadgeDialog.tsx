...
export const BadgeManagementDialog: React.FC<BadgeManagementDialogProps> = ({
    ...
    return (
        <>
                        ...
                        <div className="space-y-3">
                            {currentBadges.length === 0 ? (
                                ...
                                currentBadges.map((badge) => (
                                    <div
                                        key={badge}
                                        className="flex items-center justify-between p-3 bg-neutral-700/50 rounded-lg group hover:bg-neutral-700 transition-colors"
                                    >
                                        <div className="flex items-center gap-2">
                                            {badgeIcons[badge] || <Badge size={20} className="text-neutral-400" />}
                                            <span className="text-neutral-200">{badge}</span>
                                        </div>
                                        <button
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {filteredAvailableBadges.map((badge) => (
                                <button
                                    ...
                                    className="p-3 bg-neutral-700/50 rounded-lg hover:bg-neutral-700 transition-colors flex items-center justify-center gap-2 group"
                                >
                                    <Plus size={20} className="text-neutral-400 group-hover:text-neutral-400" />
                                    <span className="text-neutral-300 group-hover:text-neutral-100">
                                        {badge}
                                    </span>
                                </button>
                            ))}
            ...
            >
                <div>
                    <p className="text-neutral-400">Möchtest du wirklich das Badge <strong>{confirmDialog.badge}</strong> von {username} entfernen?</p>
                    <Button
